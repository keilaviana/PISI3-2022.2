...
# Combina o DataFrame original com os rótulos de cluster
clustered_data = pd.concat([df, dataset[['nutrient_name']]], axis=1)

# Agrupa por rótulo de cluster e nome do nutriente, contando as ocorrências
nutrient_counts_by_cluster = clustered_data.groupby(['cluster_label', 'nutrient_name']).size().reset_index(name='count')

# Ordena os resultados em ordem decrescente de contagem
nutrient_counts_by_cluster = nutrient_counts_by_cluster.sort_values(['cluster_label', 'count'], ascending=[True, False])

# Itera sobre os grupos e exibe as informações
for cluster_label, group in nutrient_counts_by_cluster.groupby('cluster_label'):
    print(f"Cluster {cluster_label}:")
    for index, row in group.iterrows():
        print(f"  Nutriente: {row['nutrient_name']}, Quantidade: {row['count']}")
    print()