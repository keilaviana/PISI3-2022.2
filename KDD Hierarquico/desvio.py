...
feature_columns = [col for col in df.columns if col.startswith('description_') or col.startswith('nutrient_name_') or col.startswith('nutrient_unit_')]

cluster_stats = df.groupby('cluster_label')[feature_columns].mean().join(df.groupby('cluster_label')[feature_columns].std(), rsuffix='_std')

new_columns = [f'{col}_mean' for col in feature_columns] + [f'{col}_std' for col in feature_columns]
cluster_stats.columns = new_columns

df = df.join(cluster_stats, on='cluster_label')
